...
1. Conectar ao banco de dados no arquivo “.env” da raiz da pasta do laravel 
...
1. Criar Model do produto juntamente com a migration, no CMD:

```jsx
php artisan make:model Produto -m
```

1. Adicionar campos ao Schema do produto no arquivo “database/migrations/…create_produtos_table.php”. Aqui o nosso produto terá os atributos, nome, descrição e imagem, além dos campos padrões de id e timestamps:
...
1. Rodar migration, após isso nossa tabela de produtos estará criada:

```jsx
php artisan migrate
```

1. Editar Model da tabela de produtos, para informar que os atributos “nome, descricao e imagem” podem ser ser atibuídos aos criar ou atualizar nosso modelo, faça isto no arquivo “app/Models/Produto.php”:
...
1. Criar controller dos produtos, no cmd rodar:

```jsx
php artisan make:controller Api/ProdutoController
```

1. Adicinar rota no arquivo “routes/api.php”;
...
1. Criar o método de index do controller, que dará acesso a todos os produtos da aplicação no arquivo “app/Http/Controllers/Api/ProdutoController.php”:
...
1. Criar request (Referente ao envio de dados do cliente para o servidor) para armazenar os produtos no banco de dados, digitar no CMD: 

```jsx
php artisan make:request StoreProdutoRequest
```

1. Permitir o usuário inserir dados, aqui permitiremos que qualquer usuário faça requests, mas em uma aplicação real nós utilisariamos de lógicas para permitira que apenas usuários com permissão fizessem requests. No arquivo “app/Http/Requests/StoreProdutoRequest.php” vamos substituir o false pelo true: 
...
1. Criar regras de validação para a inserção de novos dados: app\Http\Requests\StoreProdutoRequest:
...
1. Criar método para armazenamento dos dados, no nosso controller adicionar: 
...
1. Criar controller de update, para atualizar os nossos dados. Agora, ao enviarmos um json para a rota “[localhost/ecommerce-example-api/public/api/produtos](http://localhost/ecommerce-example-api/public/api/produtos)/{id}”, com o método PUT, passando as informações do produto novo: 
...
1. Criar resource para mostrar todos os produtos, no CMD: 

```jsx
php artisan make:resource ProdutoResource
```

1. Definir campos que deseja retornar ao requisitar as informações dos produtos no app\Http\Resources\ProductResource;
...
1. Colocar o método do index para retornar todos os produtos: 
...
1. Criar o método para deletar um produto 
